...
type ty =
  ...
  | Ta of ty * ty
  | Tg of int
  | Td of name * (constructor Array.t)
and constructor =
  { c_name : name
  ; c_args : ty Array.t
  ; c_discrim : int
...
let as_function t =
  ...
  | Ta (Ta (arrow, a), b) ->
    if arrow == arrow_t
    then return (a,b)
    else Error ("not a function type")
...
let mk_datatype name cons =
  let mk_c i (c_name, args) =
    { c_name=c_name
    ; c_args=args
    ; c_discrim=i
    }
  in
  let cs = Array.of_list @@
    List.mapi mk_c cons in
  Td (name, cs)
...
let rec datatype_size cons =
  let helper acc con =
    ...
    Int.max acc size
  in
  let inner_c = Array.fold_left helper 0 cons in
  1 + inner_c
